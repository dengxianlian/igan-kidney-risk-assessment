(() => {
  ...
  const MODEL_DEFS = {
    uBASE: {
      id: "uBASE", name: "IgAN-uBASE", nameEn: "IgAN-uBASE", subtitle: "基础变量 + 尿标志物", subtitleEn: "Core variables + urinary marker", interpretation: "IgA 肾病", interpretationEn: "IgA nephropathy", intercept: 0.75,
    ...
    base: {
      id: "base", name: "IgAN-BASE", nameEn: "IgAN-BASE", subtitle: "基础临床变量", subtitleEn: "Core clinical variables", interpretation: "IgA 肾病", interpretationEn: "IgA nephropathy", intercept: -3.05,
    ...
    oxt: {
      id: "oxt", name: "IgAN-OxT", nameEn: "IgAN-OxT", subtitle: "肾小管-间质损伤风险", subtitleEn: "Tubulointerstitial injury risk", interpretation: "肾小管萎缩/间质纤维化", interpretationEn: "Tubular atrophy / interstitial fibrosis", intercept: 9.28 - 1.54635793205125,
  ...
  function renderForm(model) {
    ...
    els.formulaText.textContent = lang === "zh" ? model.formula : model.formulaEn;
  ...
  function renderNomogram(model, values, result, tier) {
    ...
    scale.ranges.forEach((range, index) => { const field = range.field; const y = top + (index + 1) * rowHeight; const currentC = field.beta * values[field.key]; const points = (currentC - range.minC) * scale.pointPerA; const maxPoints = range.span * scale.pointPerA; text(17, y + 4, field.symbol, { fill: "#284c5a", "font-size": 11, "font-weight": "700" }); text(17, y + 19, fieldLabel(field), { fill: "#8ca0a7", "font-size": 9 }); line(left, y, left + axisWidth, y, { stroke: "#a7b8bd", "stroke-width": 1.2 });
      for (let i = 0; i <= 10; i += 1) { const x = xAt(i / 10); line(x, y, x, y + (i % 2 === 0 ? 7 : 4), { stroke: "#79929a", "stroke-width": 1 }); const rawRatio = field.beta < 0 ? 1 - i / 10 : i / 10; const rawValue = range.minValue + rawRatio * (range.maxValue - range.minValue); let label = field.type === "select" ? (rawValue > .5 ? (lang === "zh" ? "男" : "Male") : (lang === "zh" ? "女" : "Female")) : formatNumber(rawValue, field.decimals); if (i === 0 || i === 5 || i === 10) text(x, y + 21, label, { "text-anchor": "middle", fill: "#708890", "font-size": 9 }); }
      const mx = pointX(points); line(mx, y - 9, mx, y + 11, { stroke: "#13a39e", "stroke-width": 2.2 }); add("circle", { cx: mx, cy: y, r: 5, fill: "#fff", stroke: "#13a39e", "stroke-width": 2.2 }); add("circle", { cx: mx, cy: y, r: 2, fill: "#13a39e" }); text(mx, y - 13, `${Math.round(points)} ${t("points")}`, { "text-anchor": "middle", fill: "#087278", "font-size": 9, "font-weight": "800" }); text(left + axisWidth, y + 21, `${formatNumber(field.beta > 0 ? range.maxValue : range.minValue, field.decimals)} → ${Math.round(maxPoints)} ${t("points")}`, { "text-anchor": "end", fill: "#9aacb0", "font-size": 8 });
    });
    ...
    const probabilityY = totalY + rowHeight; text(17, probabilityY + 4, t("riskProbability"), { fill: "#284c5a", "font-size": 11, "font-weight": "800" }); const bandY = probabilityY - 8; const bands = [{ from: 0, to: .15, color: "#77c887" }, { from: .15, to: .4, color: "#b5dc9d" }, { from: .4, to: .7, color: "#efd86b" }, { from: .7, to: .9, color: "#f2a06b" }, { from: .9, to: 1, color: "#d85b61" }]; bands.forEach((band) => { const x1 = xAt(aRatio(scale.aMin + logit(band.from === 0 ? .0001 : band.from))); const x2 = xAt(aRatio(scale.aMin + logit(band.to === 1 ? .9999 : band.to))); add("rect", { x: Math.min(x1, x2), y: bandY, width: Math.abs(x2 - x1), height: 14, fill: band.color, opacity: .82 }); }); [0, .15, .4, .7, .9, 1].forEach((p) => { const x = xAt(aRatio(scale.aMin + logit(p === 0 ? .0001 : p === 1 ? .9999 : p))); line(x, bandY + 14, x, bandY + 21, { stroke: "#4e6873", "stroke-width": 1.2 }); text(x, bandY + 33, `${Math.round(p * 100)}%`, { "text-anchor": "middle", fill: "#536e79", "font-size": 9 }); }); const pX = xAt(aRatio(result.a)); line(totalX, totalY + 8, pX, bandY - 8, { stroke: tier.color, "stroke-width": 1.5, "stroke-dasharray": "4 4", opacity: .72 }); add("circle", { cx: pX, cy: bandY - 8, r: 4.7, fill: tier.color }); line(pX, bandY - 8, pX, bandY + 19, { stroke: tier.color, "stroke-width": 2.5 }); text(pX, bandY - 17, `${(result.p * 100).toFixed(1)}%`, { "text-anchor": "middle", fill: tier.color, "font-size": 10, "font-weight": "850" }); text(left + axisWidth, probabilityY + 33, t("scoreMapping"), { "text-anchor": "end", fill: "#8da0a7", "font-size": 8 });
  }
